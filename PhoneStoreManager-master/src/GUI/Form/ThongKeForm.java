...
//        tabs.addTab("Thống kê ver2", getIcon("icons8_company_30px.png"), tk2);
    ...
    private ImageIcon getIcon(String filename) {
        return new ImageIcon(getClass().getResource("/com/PhoneStoreManager/image/" + filename));
    }
}

class ThongKe_P extends JPanel {
    ...
    Table tbThongKeHoaDon = new Table();
    Table tbThongKePhieuNhap = new Table();

    Table tbKetQuaHoaDon = new Table();
    Table tbKetQuaPhieuNhap = new Table();
...
//        tbKetQuaPhieuNhap.setAlignment(6, JLabel.RIGHT);
        ...
        tabDoiTuongThongKe.addTab("Tổng", getIcon("thongke1.png"), plThongKeSoLuong);
        tabDoiTuongThongKe.addTab("Bán ra", getIcon("shop1.png"), plThongKeHoaDon);
...
//        qlspBUS.readDB();
    ...
    private void onChangeThongKeBanHang() {
        ...
        for (HoaDon hd : qlhdBUS.search("Tất cả", "", mcd.getNgayTu(), mcd.getNgayDen(), -1, -1)) {
            float tongTien = 0;
            ArrayList<ChiTietHoaDon> dscthd = qlcthdBUS.getChiTietHoaDon(hd.getMaHD());

            if (dscthd.size() > 0) {
                NhanVien nv = qlnvBUS.getNhanVien(hd.getMaNV());
                KhachHang kh = qlkhBUS.getKhachHang(hd.getMaKH());
                ...
                tbThongKeHoaDon.addRow(new String[]{
                    hd.getMaHD() + " (" + hd.getNgayLap().toString() + ")",
                ...
                for (ChiTietHoaDon cthd : dscthd) {
                    SanPham sp = qlspBUS.getSanPham(cthd.getMaSP());
    ...
    private void onChangeThongKeNhapHang() {
        ...
        ArrayList<NCC> dsncc = new ArrayList<>(); // danh sách lưu các ncc có làm phiếu nhập
        ArrayList<SanPham> dssp = new ArrayList<>(); // lưu các sản phẩm
        ...
        for (PhieuNhap pn : qlpnBUS.search("Tất cả", "", mcd.getNgayTu(), mcd.getNgayDen(), -1, -1)) {
            float tongTien = 0;
            ArrayList<ChiTietPhieuNhap> dsctpn = qlctpnBUS.getChiTietPhieuNhap(pn.getMaPN());

            if (dsctpn.size() > 0) {
                NhanVien nv = qlnvBUS.getNhanVien(pn.getMaNV());
                NCC ncc = qlnccBUS.getNCC(pn.getMaNCC());
